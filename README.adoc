= MCP with OpenShift - GitOps Charts (User Provisioning)
:toc: macro
:toclevels: 3
:icons: font

Helm charts and agent code for the MCP with OpenShift lab, deployed per-user via ArgoCD ApplicationSets during **user provisioning**.

This repo is referenced by the `ocp4_workload_ocpsandbox_mcp_user` role in the https://github.com/rhpds/rhpds.ocpsandbox_mcp_with_openshift[rhpds.ocpsandbox_mcp_with_openshift] collection. ArgoCD deploys these charts into per-user namespaces.

toc::[]

== Overview

The MCP GitOps project provides Helm charts for deploying per-user MCP infrastructure and an intelligent agent that integrates with Tekton pipelines to automatically:
...
│       └── test_mcp_client.py
│
└── tenant/                     # Per-user Helm charts (deployed via ArgoCD)
...
----
# Deploy the AI agent
helm install agent tenant/agent -n your-namespace

# Deploy MCP servers
helm install mcp-openshift tenant/mcp-openshift -n your-namespace
helm install mcp-gitea tenant/mcp-gitea -n your-namespace
----
